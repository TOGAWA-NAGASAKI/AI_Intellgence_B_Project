...
def train():
    ...
    for run in range(1, 501):  # 500 times
        ...
        while True:
            ...
            if done:
                ...
                if current_score >= best_score and current_score >= 400:
                    best_score = current_score
                    dqn_solver.save_weights(f"models/best_double_dqn_score{best_score}")
                    print(f"New Best Score: {best_score} Saved")
                break

            if run % 50 == 0:
                dqn_solver.save_weights(f"checkpoints/checkpoint_ep{run}")
                print(f"Saved: checkpoint_ep{run}")
